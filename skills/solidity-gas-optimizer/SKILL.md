...
4. **Report location.** Ask for a preferred output location; it can override the Phase 5 default (a `gas-reports/` directory in the audited repo, plus an archive copy in the optimizer repo root).

Once scope is settled, if your agent exposes a task or to-do list, open one with the seven phases below as its items (Discover, Baseline, Inventory, Scan, Verify loop, Draft report, Tradeoff challenge), and keep it current as you go: mark a phase in-progress when you enter it and completed when its artifacts exist on disk. This gives the run a visible progress checklist. When no such facility exists, skip it; it is a display aid, not a pipeline step.
...
Fill `templates/report.md` as `report.md` in the run directory; the template defines every section and population (findings, advisory, coverage-gap, rejected) and their rules, including that coverage-gap is its own population and never merged into rejected. The run directory is the location the user named in Phase 0, else a `gas-reports/<target>-<date>/` inside the audited repo. Also copy the finished run directory into a `gas-reports/` at the root of the optimizer repository that ships this skill, not inside the skill's own subfolder: resolve that root as the git top-level of this skill's directory (the directory holding `README.md` and `skills/`). That copy is best-effort, so when the skill is not installed inside such a repository or the root is not writable, skip it and warn instead of failing. Hand the user the run directory path explicitly at the end, never leave it only in scratch space.

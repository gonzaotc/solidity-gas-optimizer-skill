...
For each hot function, walk its body once and write down what it spends on the EVM as a short per-function tally, then interrogate each entry. Recording the tally, not just thinking it, is what makes this cost-accounting rather than a hunch: a resource you never wrote down is one you can silently skip. The tally is scanner scratch, not a formal deliverable. Trace these resources:
...
- **External calls (CALL/STATICCALL)** — cold target adds 2600; each call re-warms nothing for a fresh address. Look for the same address called repeatedly; data fetched per call that could be fetched once; `extcodesize`/existence checks paid more than once.
...
- **Memory** — expansion cost grows with the high-water mark: linear per word, plus a quadratic term that dominates only at large sizes. Look for arrays or structs allocated larger than used; copies (`memory` params, `abi.encode`) that a `calldata` or storage pointer would avoid.
...
After the tally, apply the anchor question to each line: which resource is paid **more often, colder, wider, or earlier** than the semantics require? Each "yes" is a candidate.
...
- **first vs subsequent** — a one-time zero→nonzero SSTORE, a lazy init, a cache that warms after call one: is a per-call cost actually amortizable, or is a one-time cost being paid every time?
- **constructor vs runtime** — a value derived at runtime but fixed for the deployment could be `immutable` (set in the constructor, baked into runtime code) or `constant` (inlined at each use site); either is read from code instead of storage, dropping the SLOAD. Conversely, code or data that bloats every deployment but runtime never needs.

...
- **Risks**: sentinel values obscure meaning and invite off-by-one logic bugs. For the classic reentrancy-lock use case this trick is obsolete on chains with EIP-1153 (Dencun): transient storage (TSTORE/TLOAD, 100 gas, auto-cleared) is strictly better; solc 0.8.24+ supports it. Changing an existing flag encoding on a deployed/upgradeable contract changes stored-state semantics and is breaking (Tier C there).
...
```
- **Savings**: one slot instead of two halves both the cold-access and SSTORE costs when both values touch in one transaction. Manual packing beats compiler auto-packing slightly because it does one explicit SSTORE instead of letting the EVM mask-and-merge per field.
...
- **Risks**: manual bit-twiddling hurts readability and auditability. Any repacking changes storage layout: on an already-deployed or upgradeable contract this corrupts existing state and is a breaking change (Tier C in that context).
...
- **Risks**: reordering members of a struct used in an already-deployed or upgradeable contract's storage rearranges stored data and is a breaking layout change (Tier C there). Also changes positional-constructor argument order at call sites, and packed members cost extra masking when accessed individually.
...
- **Preconditions**: content length must be bounded below 32 bytes by construction, not by hope; the bytes32/assembly variant needs an explicit length guard on write.
...
- **Risks**: converting an existing storage variable removes its slot, shifting subsequent variables: breaking for deployed/upgradeable layouts (Tier C there). In proxy patterns, immutables live in the implementation's code, so all proxies share the value set at implementation deploy. None for new non-proxy code.
...
- **Risks**: out-of-range keys silently return zero instead of reverting, converting a loud bug into silent state corruption; length must be tracked separately if ever needed. Swapping the type on a deployed/upgradeable contract changes slot derivation for the data and is breaking (Tier C there).
...
- **Preconditions**: participants need dense sequential indices (e.g. merkle-leaf positions); address-keyed flags must first be mapped to indices. Wins require multiple flags landing in the same word.
- **Risks**: index-assignment machinery adds complexity, and bit arithmetic is easier to get wrong than a bool. Migrating an existing bool mapping on a deployed/upgradeable contract abandons old state and is breaking (Tier C there).
...
- **Risks**: mixed-width arithmetic invites silent truncation at cast sites; comparisons against unshrunken `uint256` values need care. Narrowing a field in an already-deployed or upgradeable contract rewrites the slot layout and is breaking (Tier C there).

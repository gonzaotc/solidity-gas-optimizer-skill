...
- **Risks**: sentinel values obscure meaning and invite off-by-one logic bugs. For the reentrancy-lock use case this technique is obsolete on chains with EIP-1153 (Dencun): transient storage (TSTORE/TLOAD, 100 gas, auto-cleared) is strictly better; solc 0.8.24+ supports it. Changing an existing flag encoding on a deployed/upgradeable contract changes stored-state semantics and is breaking (Tier C there).

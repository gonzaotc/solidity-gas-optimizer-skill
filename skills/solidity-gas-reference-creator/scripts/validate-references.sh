...
#   - every card body has the seven required schema fields: Kind, Detect,
#     Hint, Transform, Savings, Preconditions, Risks (Source is optional)
...
for f in "$dir"/*.md; do
  ...
  awk -v file="$name" '
    function check(   n, req, i) {
      n = split("**Kind**,**Detect**,**Hint**,**Transform**,**Savings**,**Preconditions**,**Risks**", req, ",")
      for (i = 1; i <= n; i++)

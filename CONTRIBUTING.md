...
Use the `solidity-gas-reference-creator` skill (install it like the audit skill: symlink `skills/solidity-gas-reference-creator` into `~/.claude/skills/`): describe the technique, or just paste a snippet or writeup, and it deduplicates against the catalog, routes and numbers the card, regenerates `INDEX.md`, and runs the validator. Naming a source is optional; when you do, it also records the mapping in `SOURCES.md`. It refuses a technique whose mechanism is already carded and reports the existing card instead.

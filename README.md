...
- **A gas measuring tool: Foundry or Hardhat V2/V3.** For Foundry, `forge` on PATH (`forge snapshot` is preferred when both are present); for Hardhat, a gas reporter, which covers only the functions the tests exercise: Hardhat 3 reports gas natively via `--gas-stats`, and Hardhat 2 needs `hardhat-gas-reporter` installed.
...
│   ├── catalog/                  technique catalog (INDEX.md, SOURCES.md, SOURCE-LOG.md, cards)
